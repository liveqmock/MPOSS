...
<div class="pageContent">
	<table class="list" width="100%" layoutH="38">
		<thead>
		    <tr>
		        <th colspan="10">${transDate}${myfn:getConstantName('TRANS_ITEM', transItem)}${myfn:getConstantName('TRANS_DIRE', transDire)}详情</th>
		    </tr>
			<tr>
				<th>支付时间</th>
				<th>交易类型</th>
		...
		<tbody>
			...
			<tr>
			    ...
			    <td style="color:green;">+${myfn:fromF2YForShow(trans.transPrice)}</td>
			    </c:when>
			    <c:otherwise>
			    <td style="color:red;">${myfn:fromF2YForShow(trans.transPrice)}</td>
			    </c:otherwise>
			    </c:choose>
			...
			<tr>
			    ...
			    <td style="color:green;">+${myfn:fromF2YForShow(totalTransPrice)}</td>
			    </c:when>
			    <c:otherwise>
			    <td style="color:red;">${myfn:fromF2YForShow(totalTransPrice)}</td>
			    </c:otherwise>
			    </c:choose>
			    <td></td>
			    <td></td>
