...
public class AppConfiguration {
	...
	public static synchronized void init(String configFile)
		...
		System.out.println("configFile " + configFile); //D:\Workspaces\MPOSS\WebRoot\WEB-INF\etc\conf\app-config.xml
		...
		if (appConf == null) {
			...
			String basePath = configFile.substring(0, pIndex); //D:\Workspaces\MPOSS\WebRoot\WEB-INF\etc\conf
	...
	private void fetchTimerTask(String basePath) {
		String path = this.conf.getString("app.timer.task.config"); // /utils/app-timer-task.xml
		File file = new File(basePath + path); // D:\Workspaces\MPOSS\WebRoot\WEB-INF\etc\conf/utils/app-timer-task.xml
		if(file.exists()) {
			TaskFactory factory = TaskFactory.getFactory();
			factory.initTasks(basePath + path); //初始化任务
